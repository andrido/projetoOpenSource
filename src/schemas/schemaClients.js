...
const schemaClients = joi.object({
    ...
    cpf: joi.string().regex(/^\w+$/).min(11).max(11).required().messages({
        ...
        'string.pattern.base': 'O campo cpf não pode ter espaço em branco'
    }),
    cep: joi.string().regex(/^\w+$/).min(8).max(8).messages({
      ...
      'string.pattern.base': 'O campo cep não pode ter espaço em branco'
    }),
